...
def get_last_five_articles():
    response = {'data': None, 'error': None, 'success': False}
    try:
        # Query para obtener los últimos 5 artículos que cumplen con los criterios
        ...
        # Preparar la lista para almacenar los datos extendidos de los artículos
        extended_articles = []
        for article in articles:
            # Obtener el nombre del bot usando el bot_id del artículo
            bot = Bot.query.get(article.bot_id)
            bot_name = bot.name if bot else None

            # Obtener el nombre de la categoría usando el category_id del bot
            category = Category.query.get(bot.category_id) if bot else None
            category_name = category.name if category else None

            # Construir el diccionario extendido del artículo
            extended_article = {
                'id': article.id,
                'title': article.title,
            ...
            # Agregar el artículo extendido a la lista
