...
from config import Category, Metrics
...
def process_item(self, item):
    with self.app.app_context():
        ...
        metrics = self.data_manager.get_metrics(self.bot_id)
        if not metrics:
        ...
        try:
            ...
            if not self.process_url(link) or not datetime_checker(pub_date):
                metrics.articles_discarded += 1
                self.data_manager.save_metrics(metrics)  # Save metrics after processing
            ...
            try:
                ...
                metrics.articles_saved += 1  # Increment saved articles count
